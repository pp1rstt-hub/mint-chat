...
// ВСТАВИВ ТВОЇ КЛЮЧІ, ЯКІ МИ РОБИЛИ НА ПОЧАТКУ
    ...
    <div style={{display:'flex', height:'100vh', background:'#000', color:'#fff', fontFamily:'Arial'}}>
      ...
      <div style={{flex:1, display:'flex', flexDirection:'column'}}>
        <div style={{flex:1, overflowY:'auto', padding:'20px', display:'flex', flexDirection:'column', gap:'10px'}}>
          {messages.map(m => (
            <div key={m.id} style={{alignSelf: m.sender === name ? 'flex-end' : 'flex-start', background: m.sender === name ? '#005c4b' : '#222', padding:'10px', borderRadius:'8px'}}>
        ...
        <div style={{padding:'20px', background:'#0a0a0a', display:'flex', gap:'10px'}}>
          ...
          <button onClick={send} style={{background:'#00a884', border:'none', padding:'0 20px', color:'#fff', cursor:'pointer'}}>SEND</button>
        </div>
      </div>
    </div>
  );
